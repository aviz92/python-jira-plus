...
# pylint: disable=no-self-use,redefined-outer-name,consider-using-assignment-expr
import os
...
class TestConnection:
    def test_client_is_connected(self, jira: JiraCloud) -> None:
        result = jira.check_client_connection()
        assert result is True, "Expected active connection to Jira Cloud"

    def test_server_version_is_tuple(self, jira: JiraCloud) -> None:
        assert isinstance(jira.server_version, tuple), f"Expected tuple, got {type(jira.server_version)}"
        assert len(jira.server_version) > 0, "Expected non-empty version tuple"

    def test_cloud_uses_createmeta(self, jira: JiraCloud) -> None:
        assert (
            jira.use_createmeta is True
...
class TestGetObjectsByQuery:
    def test_returns_list_of_issues(self, jira: JiraCloud, project_key: str, issue_type: str) -> None:
    ...
    def test_returned_issues_have_basic_fields(self, jira: JiraCloud, project_key: str, issue_type: str) -> None:
        query = f'project = "{project_key}" AND issuetype = "{issue_type}"'
        result = jira.get_objects_by_query(
            query=query,
            specific_fields=BASIC_FIELDS,
    ...
    def test_invalid_jql_raises(self, jira: JiraCloud) -> None:
        with pytest.raises(JIRAError):
            jira.get_objects_by_query(query="INVALID JQL !!!", max_results=1)


class TestGetIssueByKey:
    def test_json_result_true_returns_dict(self, jira: JiraCloud, issue_key: str) -> None:
        ...
        assert "key" in result, "Expected 'key' in result"

    def test_json_result_false_returns_issue_object(self, jira: JiraCloud, issue_key: str) -> None:
    ...
    def test_nonexistent_issue_returns_none(self, jira: JiraCloud) -> None:
...
class TestGetAllowedValues:
    def test_returns_list_for_valid_field(
        self, jira: JiraCloud, project_key: str, issue_type: str, test_field: str
    ) -> None:
        ...
        assert result is None or isinstance(result, list), f"Expected list or None, got {type(result)}"

    def test_invalid_field_raises_value_error(self, jira: JiraCloud, project_key: str, issue_type: str) -> None:
