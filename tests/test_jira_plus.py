...
class TestResolveLegacyApi:
    def test_cloud_uses_legacy_api(self, jira_plus_cloud: JiraPlus) -> None:
        assert jira_plus_cloud.use_legacy_api is True, \
            "Cloud uses legacy createmeta API — project_issue_types is not supported for Cloud by the jira library"

    def test_on_premise_below_9_uses_legacy_api(self, jira_plus_on_premise_legacy: JiraPlus) -> None:
        assert jira_plus_on_premise_legacy.use_legacy_api is True, \
            "ON_PREMISE < 9.0 should use legacy createmeta API"

    def test_on_premise_9_and_above_uses_new_api(self, jira_plus_on_premise_new: JiraPlus) -> None:
        assert jira_plus_on_premise_new.use_legacy_api is False, \
            "ON_PREMISE >= 9.0 should use project_issue_types API"


class TestGetIssueByKey:
    def test_json_result_true_returns_raw_dict(self, jira_plus_cloud: JiraPlus) -> None:
    ...
    def test_json_result_false_returns_issue_object(self, jira_plus_cloud: JiraPlus) -> None:
        mock_issue = MagicMock()
        jira_plus_cloud.jira_client.issue.return_value = mock_issue
    ...
    def test_jira_error_returns_none(self, jira_plus_cloud: JiraPlus) -> None:
    ...
    def test_unexpected_error_returns_none(self, jira_plus_cloud: JiraPlus) -> None:
...
class TestDeleteIssue:
    def test_success_returns_true_and_calls_delete(self, jira_plus_cloud: JiraPlus) -> None:
        mock_issue = MagicMock()
        jira_plus_cloud.jira_client.issue.return_value = mock_issue
    ...
    def test_jira_error_returns_false(self, jira_plus_cloud: JiraPlus) -> None:
    ...
    def test_unexpected_error_returns_false(self, jira_plus_cloud: JiraPlus) -> None:
        jira_plus_cloud.jira_client.issue.side_effect = RuntimeError("network failure")
...
class TestValidateFields:
    def _setup_createmeta(self, jira_plus: JiraPlus, fields: dict) -> None:
    ...
    def test_valid_string_field_raises_no_error(self, jira_plus_cloud: JiraPlus) -> None:
        self._setup_createmeta(jira_plus_cloud, {
            "summary": {"schema": {"type": "string"}, "name": "Summary"},
        })

        jira_plus_cloud.validate_fields("PROJ", "Story", {"summary": "Hello"})

    def test_field_not_in_metadata_raises_value_error(self, jira_plus_cloud: JiraPlus) -> None:
        self._setup_createmeta(jira_plus_cloud, {
            "summary": {"schema": {"type": "string"}, "name": "Summary"},
        })
    ...
    def test_wrong_type_for_string_field_raises_value_error(self, jira_plus_cloud: JiraPlus) -> None:
        self._setup_createmeta(jira_plus_cloud, {
            "summary": {"schema": {"type": "string"}, "name": "Summary"},
        })
    ...
    def test_wrong_type_for_number_field_raises_value_error(self, jira_plus_cloud: JiraPlus) -> None:
        self._setup_createmeta(jira_plus_cloud, {
    ...
    def test_invalid_allowed_value_raises_value_error(self, jira_plus_cloud: JiraPlus) -> None:
        self._setup_createmeta(jira_plus_cloud, {
            "priority": {
                "schema": {"type": "string"},
                "name": "Priority",
...
class TestPaginateQueryNew:
    def test_single_page_returns_issues(self, jira_plus_cloud: JiraPlus) -> None:
        ...
        result = jira_plus_cloud._paginate_query_new(
            query="project = PROJ",
            max_results=50,
            specific_fields="*all",
            jira_err_limit=3,
    ...
    def test_multiple_pages_accumulates_all_issues(self, jira_plus_cloud: JiraPlus) -> None:
        ...
        assert result == [{"key": "PROJ-1"}, {"key": "PROJ-2"}], \
            f"Expected both pages merged, got {result}"

    def test_empty_page_returns_empty_list(self, jira_plus_cloud: JiraPlus) -> None:
        jira_plus_cloud.jira_client.enhanced_search_issues.return_value = {}
    ...
    def test_bad_request_raises_jira_error(self, jira_plus_cloud: JiraPlus) -> None:
        jira_plus_cloud.jira_client.enhanced_search_issues.side_effect = JIRAError(
            "Bad JQL", status_code=400
        )

        with pytest.raises(JIRAError):
            jira_plus_cloud._paginate_query_new(
                query="invalid JQL !!!",
                max_results=50,
                specific_fields="*all",
                jira_err_limit=3,
...
class TestGetAccountData:
    def test_success_returns_user_list(self, jira_plus_cloud: JiraPlus) -> None:
        mock_response = MagicMock()
        mock_response.json.return_value = [{"accountId": "abc123", "displayName": "Test User"}]

        with patch("python_jira_plus.jira_plus.requests.get", return_value=mock_response):
    ...
    def test_http_error_raises(self, jira_plus_cloud: JiraPlus) -> None:
        mock_response = MagicMock()
